    ...
    for global_epoch in range(len(epochs_lists)):
        for epoch in tqdm(range(test_iter), colour='GREEN', ncols=80):
            ...
            writer.add_scalar("PSNR", psnr.item(), 
                              global_step=lr_global_epochs)
            lr_global_epochs += 1
        torch.save(coordinate_embeddings, os.path.join('models', 'coordinate_embeddings.pt'))
        torch.save(direction_embeddings, os.path.join('models', 'direction_embeddings.pt'))
        torch.save(coarse_model, os.path.join('models', 'coarse_model.pt'))
        torch.save(fine_model, os.path.join('models', 'fine_model.pt'))
        ...
        render.render_images(render_poses, H, W, K, near, far, 
                             ...
                             global_epoch * test_iter, dataname, gpu, embedding=embedding)    
