...
def generate_raw(z_vals, model, rays_d, rays_o, coords_embeddings, direction_embeddings, embedding, gpu=True):
    '''
    z_vals: [N_rays, N_samples]
    rays_d: [N_rays, 3]
    rays_o: [N_rays, 3]
    ...
    if embedding == 'ngp': 
        ...
        coordinates_embed = coords_embeddings(coordinates)
        viewdirs_embed = direction_embeddings(viewdirs)
        raw_outputs = model(coordinates_embed, viewdirs_embed)
...
def render_rays(z_vals, rays_d, color, sigma, gpu=True):
    ...
    z_vals_distance = torch.cat([z_vals_distance, torch.Tensor([1e10]).expand(z_vals_distance[...,:1].shape).to(device)], -1)
...
def render_images(render_poses, H, W, K, near, far, N_rays, N_samples, 
                  N_importance, coarse_model, fine_model, coords_embeddings, direction_embeddings, epoch, dataname, gpu=True, embedding='npg'):
    coarse_model.eval()
    fine_model.eval()
    ...
    imageio.mimwrite(os.path.join('dataset', 'nerf_synthetic', dataname, 'logs', str(epoch) + '_fine_network_rgb_video.mp4'), 
